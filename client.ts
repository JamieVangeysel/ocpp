...
import { CONNECTING, OPEN, CLOSING, CLOSED, WebSocket, RawData } from 'ws'
...
import { getErrorPlainObject, createRPCError, getServerName } from './util'
...
const ws_abort_states: number[] = <const>[CLOSED, CLOSING]
export type Exiting = typeof ws_abort_states[number]

const ws_states = <const>[CONNECTING, OPEN, CLOSING, CLOSED]
export type States = typeof ws_states[number]


const MSG_CALL = 2
const MSG_CALLRESULT = 3
const MSG_CALLERROR = 4
const ocpp_message_types = <const>[MSG_CALL, MSG_CALLRESULT, MSG_CALLERROR]
export type OCPPMessageTypes = typeof ocpp_message_types[number]

const ocpp_response_message_types = <const>[MSG_CALLRESULT, MSG_CALLERROR]
export type OCPPResponseMessageTypes = typeof ocpp_message_types[number]

export interface OCPP_ClientOptions {
  identity?: string
  reconnect: boolean
...
export default class OCPP_Client extends EventEmitter {
  ...
  protected _options: OCPP_ClientOptions
  ...
  reconfigure(options: OCPP_ClientOptions): void {
  ...
  async connect(): Promise<any> {
    ...
    if (this._options.query) {
      connUrl += '?' + new URLSearchParams(this._options.query).toString()
    }
   ...
   */
  sendRaw(message: Buffer | string) {
    if (this._state in ws_abort_states && this._ws) {
  ...
  async close({ code, reason, awaitPending, force }: {
    ...
    if (this._state in ws_abort_states) {
      // no-op
    ...
    if (this._state === OPEN) {
      ...
      const result = this._connectedOnce ? { code, reason } : { code: 1001, reason: 'Connection aborted' }
  ...
  async _call(method, params, options: Record<string, any> = {}) {
    ...
    const pendingCall: any = { msgId, method, params }
    if (!options.noReply) {
  ...
  _rejectPendingCalls(abortReason) {
    Array.from<any>(this._pendingCalls.values())
      .concat(Array.from<any>(this._pendingResponses.values()))
      .forEach(c => c.abort(abortReason))
  }

  async _awaitUntilPendingSettled() {
    const pendingCalls = Array.from<any>(this._pendingCalls.values())
    const pendingResponses = Array.from<any>(this._pendingResponses.values())
  ...
  _beginConnect() {
    this._connectPromise = (async () => {
      ...
      if (this._options.password != null) {
        ...
        let passwordBuffer: Buffer | string = this._options.password
  ...
  _onMessage(buffer: RawData) {
    ...
    let msgId = '-1'
    let messageType: OCPPMessageTypes | undefined
    ...
    try {
      ...
      if (!(messageTypePart in ocpp_message_types))
        throw createRPCError('MessageTypeNotSupported', 'Unrecognised message type', {})

      messageType = messageTypePart as OCPPMessageTypes
      ...
      if (messageType && !(messageType in ocpp_response_message_types)) {
    ...
    try {
      ...
      try {
        ...
        if (this.protocol === 'ocpp1.6') {
          ...
          // They wont make spelling mistakes next time, they promised!
          switch (rpcErrorCode) {
...
OCPP_Client.OPEN = OPEN
OCPP_Client.CONNECTING = CONNECTING
OCPP_Client.CLOSING = CLOSING
OCPP_Client.CLOSED = CLOSED
